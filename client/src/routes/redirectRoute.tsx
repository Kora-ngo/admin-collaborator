...
const RedirectRoute: React.FC<{children: ReactNode}> = ({children}) => {
  ...
  // 2. Welcome toast + auto-redirect when user loads
  useEffect(() => {
    if (user && token) {
      showToast({
        type: 'info',
  ...
  // 3. Detect invalid/expired token → logout
  useEffect(() => {
    if (!loading && !user && token) {
      // Token was present but /me failed → expired or invalid
  ...
  }, [loading, user, token, navigate, showToast]);
  ...
  if (token && loading && !user) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-background">
        ...
        <p className="text-lg text-primary">Loading your dashboard...</p>
      </div>
    );
  }
